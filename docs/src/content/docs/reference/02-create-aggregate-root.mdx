...
The event handlers themselves receive an event object with: `type`, `payload`, `version`, `streamId`, and `streamType`. Persisted events additionally carry their global log `position`; it is `undefined` while folding events that a command just yielded, because they have not been appended yet.
...
|---|---|---|
| `.settled(options?)` | `Promise<Stream>` | Waits for all queued commands to be persisted and all downstream listeners to finish processing. Returns the stream for chaining. Accepts an optional `{ maxRetries }` — see [Retrying on concurrency conflicts](#retrying-on-concurrency-conflicts). |
...
<Aside type="caution">
  `.settled()` cannot be called inside a process manager or a transaction — the surrounding scope settles dispatched streams automatically when it completes. To configure those auto-settles, declare defaults on the scope instead: [`createProcessManager(name, { settled })`](/reference/01-create-event-store/#createprocessmanagername-options) or [`transaction(handler, { settled })`](/reference/01-create-event-store/#transactionhandler-options).
</Aside>

### Retrying on concurrency conflicts

When two writers race on the same stream version, the losing append raises a [`ConcurrencyError`](/reference/07-errors/). This is normally prevented by the [lock](/reference/01-create-event-store/#concurrency--locking), but it can still surface in the residual cases the lock does not cover — a lock TTL lapsing mid-command, or single-instance writes racing the storage adapter's optimistic-concurrency check.

Pass `maxRetries` to `.settled()` to recover automatically: on a `ConcurrencyError` the stream is **reloaded** and the dispatched commands are **re-applied** against the current version, up to `maxRetries` times, before the error is surfaced.

```typescript
const settings = settingsRoot.loadStream('global')
settings.update({ theme: 'dark' })

// Reload + re-apply up to 5 times if another writer wins the race.
await settings.settled({ maxRetries: 5 })
```

| Option | Type | Default | Description |
|---|---|---|---|
| `maxRetries` | `number` | `0` | Number of reload-and-re-apply attempts after a `ConcurrencyError`. `0` surfaces the error immediately (the default). |

A retry re-applies the commands within the async context captured at the dispatch site, so `AsyncLocalStorage`-based state (request metadata, tenant or branch scopes, …) observed by command handlers and `postProcessEvent` is the same as on the first attempt — even when the retry is triggered later from a different context, such as a process manager's or transaction's auto-settle.

<Aside type="caution">
  Only the append is retried — command handlers are re-run against the reloaded state. They must be **pure functions of loaded state and their arguments**; a handler with side effects (sending mail, calling an external API) would repeat them on each attempt. Leave `maxRetries` at `0` when you *want* the error to surface, such as a fixed-id `newStream(...)` used for idempotent create/dedup.
</Aside>
