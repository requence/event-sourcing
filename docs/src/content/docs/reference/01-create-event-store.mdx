...
The lock and the storage adapter's version check are two layers of the same story: the lock **avoids** the race by serializing writers, and the version check **catches** it if a race slips through (raising a [`ConcurrencyError`](/reference/07-errors/)). A race can still slip through when a lock's TTL lapses mid-command, or in single-instance deployments where no distributed lock is configured.

For those residual cases, opt in to recovery at the call site with [`settled({ maxRetries })`](/reference/02-create-aggregate-root/#retrying-on-concurrency-conflicts): the stream is reloaded and the commands re-applied against the current version instead of the error surfacing.
...
Inside a [process manager](/reference/04-create-process-manager/) or a [transaction](#transactionhandler-options), `settled()` cannot be called — the surrounding scope settles dispatched streams automatically when it completes. Declare the retry defaults on that scope instead: [`createProcessManager(name, { settled })`](#createprocessmanagername-options) or [`transaction(handler, { settled })`](#transactionhandler-options).
...
|---|---|---|
| `settled` | `SettledOptions` | Defaults applied when the process manager auto-settles dispatched streams — see [Retrying on concurrency conflicts](/reference/02-create-aggregate-root/#retrying-on-concurrency-conflicts). |

<Aside type="caution">
  A process-manager-wide default asserts that **all** commands its handlers dispatch are pure functions of loaded state and their arguments — a handler added later inherits the retry behaviour invisibly.
</Aside>
...
| Option | Type | Description |
|---|---|---|
| `settled` | `SettledOptions` | Defaults applied when the transaction auto-settles dispatched streams — see [Retrying on concurrency conflicts](/reference/02-create-aggregate-root/#retrying-on-concurrency-conflicts). |
